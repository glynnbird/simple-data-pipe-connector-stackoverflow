...
var stackoverflowKey = '3NZK7gtp7RsW0DQeVs)AWA((';
var fs = require('fs');
...
function oAuthSampleConnector( parentDirPath ){
  ...
  this.getSampleDataSetList = function(pipe, done) {
      ...
      require('request').get(requestOptions, function(err, response, body) {
          if(err) {
          ...
          // unzip the response
          require('zlib').gunzip(body, function(err, dezipped) {
              if(err) {
                  // there was a problem with the request; abort processing
                  // by calling the callback and passing along an error message
                  return done('Fetch request err: ' + err, null);
